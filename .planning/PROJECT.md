...
**Shipped:** v1.0 (2026-05-07) — see `.planning/milestones/v1.0-ROADMAP.md` and `.planning/MILESTONES.md`.

**Verification:** All 4 Phase 7 success criteria signed off (SC-1 ✓, SC-2 OPTIONAL/smoke-only, SC-3 ✓ on AWS Graviton, SC-4 ✓). All 31 traced requirements complete.

**Repository layout:** `lightweight/` is an in-tree sibling Maven project alongside the main TBMQ codebase in this repo. Decision is "Deferred — stays in-tree through R1 + R2; revisit post-R2".

**Distribution:** Multi-arch (amd64 + arm64) image built via `docker buildx` and pushed to test registry `dlandiak2110/tbmq-lightweight:1.0.0` and `:latest` (OCI manifest index `sha256:98554b72...`). Production push to `thingsboard/tbmq-lightweight` is deferred — not a v1.0 gate.

**Branch:** `lightweight/v1` on `origin` (renamed from `codebase-map` 2026-05-07). Not merged to `main` per user decision; integration branch for the lightweight project.

**Planning state:** `.planning/STATE.md` status is `Awaiting next milestone`. `.planning/REQUIREMENTS.md` was archived to `milestones/v1.0-REQUIREMENTS.md`; a fresh REQUIREMENTS.md will be generated by the next `/gsd-new-milestone` cycle.

## Next Milestone Goals (R2 / v1.1)

Likely R2 scope (sourced from "Out of Scope — deferred to R2" above and STATE.md Blockers/Concerns). Confirm via `/gsd-new-milestone`:

- **Persistent sessions** — offline QoS 1/2 message queuing (PERS-01). Identified in `.planning/STATUS.md` as the biggest user-visible R1 gap vs. Mosquitto.
...
- **RocksDB schema-version / migration framework** — `metadata` column family with schema version key. Pre-R2 hygiene; required before any R2 schema change to keep upgrades clean.

**Deferred to post-R2 (do NOT raise as R2 work):**
- Repository extraction (`lightweight/` → its own repo).
- `lightweight/README.md` (quick-start, limitations, decision matrix vs. standard TBMQ).
- Production multi-arch push to `thingsboard/tbmq-lightweight` Docker Hub org.
- Merge `lightweight/v1` to `main`.
- CI workflow `mvn -f lightweight/pom.xml verify` on PRs (dropped for v1.0; revisit if regression cost grows).
...
---
*Last updated: 2026-05-07 after Milestone v1.0 archival — SC-3 ARM64 verified on AWS Graviton, multi-arch image pushed to test registry, all 7 phases shipped.*
